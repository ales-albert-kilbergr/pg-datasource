...
export function matchUniqueConstraintViolationError<C extends object>(
  constraint: string | RegExp,
) {
  const isConstraintViolation =
    matchConstraintViolationError<object>(constraint);

  return (ctx: SqlQuery.ErrorContext<object>): false | { conflict: C } => {
    if (isConstraintViolation(ctx) === true && ctx.error.code === '23505') {
